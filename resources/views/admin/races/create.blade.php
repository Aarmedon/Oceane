...
<div class="container mx-auto p-4 max-w-3xl">
    <h1 class="text-2xl font-bold mb-4">Nouvelle race</h1>
    ...
    <form method="POST" action="{{ route('admin.races.store') }}" class="space-y-4" enctype="multipart/form-data">
        @csrf

        <div>
            <label class="block font-medium">Name</label>
            <input name="name" type="text" value="{{ old('name') }}" class="border p-2 rounded w-full" required>
        </div>
        ...
        <div>
            ...
            <textarea name="description" class="border p-2 rounded w-full" rows="4">{{ old('description') }}</textarea>
        </div>

        <div>
            <label class="inline-flex items-center gap-2">
                <input name="is_playable" type="checkbox" value="1" {{ old('is_playable') ? 'checked' : '' }}>
                <span>Jouable</span>
            </label>
        </div>
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 mt-1">Formats recommandés: PNG, JPG. L'image sera stockée dans <code>storage/app/public/races</code> et servie via <code>/storage</code>.</p>
        </div>
...
<script>
    (function(){
        ...
        // If server sent back old inputs with errors, rebuild them
        @php
            $old = old('modifiers', []);
        @endphp
        @if(!empty($old))
            @foreach($old as $i => $m)
                addRow(@json($m['key'] ?? ''), @json($m['value'] ?? ''));
            @endforeach
        @else
